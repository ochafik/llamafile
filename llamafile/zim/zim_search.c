...
uint32_t zim_find_title_prefix(zim_archive *archive, const char *prefix) {
    ...
    while (lo < hi) {
        ...
        uint32_t entry_idx = archive->title_ptrs[mid];

        if (!zim_get_entry_by_index(archive, entry_idx, &entry)) {
            return UINT32_MAX;
        }

        // Compare title with prefix (case insensitive)
        int cmp = 0;
        const char *t = entry.title;
        const char *p = prefix;
        while (*p && *t) {
            int tc = tolower((unsigned char)*t);
            int pc = tolower((unsigned char)*p);
            if (tc != pc) {
                cmp = tc - pc;
                break;
            }
        ...
        if (cmp == 0 && *p) {
            cmp = -1;  // Title is shorter than prefix
        }
